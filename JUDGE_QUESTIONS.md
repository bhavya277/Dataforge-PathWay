...
**Answer:** By standard linear algebra, the maximum number of mutually orthogonal non-zero vectors in $\mathbb{R}^d$ is $d$. When $N > d$, nonzero vectors in $\mathbb{R}^d$ cannot all be mutually orthogonal, so some non-zero off-diagonal projections must occur. These projections can contribute cross-talk to linear retrieval, although individual contributions may partially cancel. Thus, we distinguish the geometric constraint (orthogonality is impossible), the existence of non-zero projections, and the actual summed cross-talk magnitude.
...
**Answer:** Everything in the primary 7-step journey is computed live in browser: the recurrence state update $S_t$, matrix heatmaps, query retrieval $y_t = S_t q$, Gram matrix $G$, target/cross-talk decompositions, and float64 residual checks. Slider adjustments re-evaluate the full linear algebra engine interactively in the browser.

### 11. What is precomputed?
**Answer:** Only the reference benchmark distributions in `data/experiment_benchmarks.json` generated by `generate_experiment_data.py`. The benchmark dataset contains 50 configurations: 1 deterministic orthogonal baseline, plus 20 correlation-sweep, 20 load-sweep, and 9 dimension-sweep configurations. The three sweep experiments use 50 Monte Carlo trials per configuration. These provide stable reference baselines on the Stress Test tab. No live UI simulation passes off precomputed traces as real-time computation. Every parameter slider change ($d, N, \rho, \lambda$) recomputes the state matrix $S_t$ from raw Float64 vector outer products live in the client.
...
**Answer:** BDH is a multi-layer neural architecture with locally interacting neuron particles, synaptic plasticity, and sparse positive activations. Our toy abstraction is a single-layer visual demonstration model applying $\text{ReLU}(v)\text{ReLU}(k)^T$ and Top-K connection gating on a small matrix to make the effect of sparse support restriction visually perceivable.
...
**Answer:** Our experiment cannot claim that non-transformer architectures face an insurmountable barrier or that BDH completely eliminates interference. Finite-dimensional capacity bounds apply to all fixed-size states; our substrate demonstrates the geometric mechanism and measured behavior in this synthetic setup.
