...
**Answer:** Only the large reference benchmark distributions (50 Monte Carlo trials per configuration across 60 parameter configurations) in `data/experiment_benchmarks.json` generated by `generate_experiment_data.py`. These provide stable reference baselines on the Stress Test tab. No live UI simulation passes off precomputed traces as real-time computation. Every parameter slider change ($d, N, \rho, \lambda$) recomputes the state matrix $S_t$ from raw Float64 vector outer products live in the client.
