...
* **The Fundamental Dilemma:** Unlike full KV caching which stores all history faithfully, a fixed-size recurrent state MUST continuously superimpose outer products:
  $$S_t = \alpha S_{t-1} + v_t k_t^T$$
  This creates an unavoidable tension between **memory capacity** and **catastrophic cross-talk interference**.
* **The BDH Breakthrough:** Dragon Hatchling (BDH) replaces dense linear superposition with sparse positive firing and localized synaptic plasticity, preventing cross-talk while maintaining linear $O(1)$ inference memory.

---

## 3. Pedagogical Persona & Framing
* **Target Learner:** AI engineers, ML researchers, and students who understand basic linear algebra (matrix multiplication) and standard attention ($Q K^T V$), but want to understand *how linear recurrence actually stores facts* and *why it breaks down*.
* **Prerequisites:** Matrix outer products ($v k^T$), dot product similarity, basic token embeddings.
* **Learning Objectives:**
  1. Derive the mathematical duality: $\text{Linear Attention } (Q K^T V) \equiv \text{Recurrent State } S_t = S_{t-1} + v_t k_t^T$.
  2. Directly visualize the $d \times d$ Fast-Weight Memory Matrix storing key-value associations.
  3. Induce catastrophic cross-talk interference by varying key orthogonality $\rho = \cos(k_i, k_j)$.
  4. Witness the exact capacity limit $N \le d$ where recall accuracy drops from 100% to 0%.
  5. Understand how BDH's sparse positive activations and Hebbian decay resolve this exact bottleneck.
...
### Central Question
> **"How does a fixed-size recurrent matrix remember key-value associations, and at what precise mathematical threshold does non-orthogonal token interference cause catastrophic recall failure?"**
